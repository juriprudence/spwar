...
// Restart sync variables
let localPlayerReadyToRestart = false;
let remotePlayersReadyToRestart = {}; // Map of playerID -> boolean

// Centralized function to manage enemy spawning based on remote players
function manageEnemySpawningBasedOnRemotePlayers() {
    // In multiplayer-only mode, we don't spawn local enemies.
    // If we're waiting for players, ensure all existing enemies are removed.
    if (typeof isWaitingForPlayers !== 'undefined' && isWaitingForPlayers) {
        if (enemies.length > 0) {
            console.log("manageEnemySpawning: Waiting for players. Removing all enemies.");
            removeAllEnemies();
        }
        return;
    }

    // Optional: If you want to spawn enemies during multiplayer, you can add that logic here.
    // For now, if no one is alive or loading, we still don't want local bots.
    if (enemies.length > 0) {
        console.log("manageEnemySpawning: Removing local enemies (Multiplayer Only Mode).");
        removeAllEnemies();
    }
}

// Function to update the waiting for players status
function updateWaitingStatus() {
    const playerCount = Object.keys(otherPlayers).length;
    const waitingOverlay = document.getElementById('waitingOverlay');

    console.log(`updateWaitingStatus: Other players: ${playerCount}`);

    // If the game is over (win or loss), don't show the waiting overlay
    ...
    if (playerCount > 0) {
        // We have at least one other player, start the game
        isWaitingForPlayers = false;
        if (waitingOverlay) waitingOverlay.style.display = 'none';
        console.log("updateWaitingStatus: Game starting (other players present)");
    } else {
        // No other players, show waiting screen and pause logic
        isWaitingForPlayers = true;
        if (waitingOverlay) waitingOverlay.style.display = 'flex';
        console.log("updateWaitingStatus: Waiting for second player...");

        // Ensure pointer is unlocked when waiting
        if (document.exitPointerLock) {
            document.exitPointerLock();
        }
    }

    // Also manage enemy spawning
...
function onEvent(code, content, actorNr) {
    ...
    switch (code) {
        ...
        */
        ...
        case PHOTON_EVENT_PLAYER_RESTART_READY: // Synchronized restart
            console.log(`Received PHOTON_EVENT_PLAYER_RESTART_READY from actorNr: ${actorNr}`);
            remotePlayersReadyToRestart[actorNr] = true;
...
function updateRemotePlayerState(playerID, data) {
    ...
    if (typeof data.hp === 'number') {
        ...
        // If player was dead and now has health (respawn)
        if (oldHp <= 0 && data.hp > 0) {
            console.log(`Player ${playerID} respawned (hp: ${data.hp}). Reloading model.`);
            loadRemotePlayerModel(playerID, playerObj.color || Math.random() * 0xffffff);
            return;
        }

        // If player health is 0 or less, hide them but don't delete from otherPlayers
        if (data.hp <= 0 && playerObj.model) {
            if (oldHp > 0) { // Only log and hide if they just transitioned to dead
                console.log(`Player ${playerID} died (hp: ${data.hp}), removing model from scene but keeping player entry.`);
                if (playerObj.model.parent) {
                    scene.remove(playerObj.model);
                }
                playerObj.model = null; // Clear model reference but keep player object for sync

                // Show Victory Screen if an opponent died
            ...
            return; // Already dead and model removed
        }
    }
...
function updateOtherPlayers(delta) {
    ...
    // Centrally manage the shared 'walk' sound for remote players
...
// Function to send restart ready event
function sendRestartReady() {
    if (!photon || !photon.isJoinedToRoom()) return;

    console.log("sendRestartReady: I am ready to restart.");
    localPlayerReadyToRestart = true;

    try {
        photon.raiseEvent(PHOTON_EVENT_PLAYER_RESTART_READY, { ready: true });
        checkAllPlayersReady();
    } catch (error) {
        console.warn("Error sending restart ready event:", error);
    }
}

// Function to check if all players are ready and reset the game if so
function checkAllPlayersReady() {
    console.log("checkAllPlayersReady: Checking players...");

    // In this game, we check simple case: are there any other players?
    // If yes, are all of them ready?
    const otherPlayerIDs = Object.keys(otherPlayers);

    // If we are alone, we can reset immediately if we are ready (though online mode usually has 2+)
    if (otherPlayerIDs.length === 0) {
        if (localPlayerReadyToRestart) {
            performFullReset();
        }
        return;
    }
    ...
    for (const id of otherPlayerIDs) {
        if (!remotePlayersReadyToRestart[id]) {
            allRemoteReady = false;
            break;
        }
    }

    if (localPlayerReadyToRestart && allRemoteReady) {
        console.log("checkAllPlayersReady: ALL PLAYERS READY. Performing reset.");
        performFullReset();
    } else {
        console.log(`checkAllPlayersReady: Waiting. Local: ${localPlayerReadyToRestart}, Others: ${allRemoteReady}`);
    }
}

function performFullReset() {
    // Reset flags
    localPlayerReadyToRestart = false;
    remotePlayersReadyToRestart = {};

    // Reset the game state
    if (typeof resetGameLocally === 'function') {
        resetGameLocally();
    }

    // Update waiting status (might show waiting screen again if someone left)
    updateWaitingStatus();
}

// Export for use in UI buttons
window.sendRestartReady = sendRestartReady;